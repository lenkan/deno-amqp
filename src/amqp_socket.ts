...
export function write(frame: OutgoingFrame): Uint8Array {
...
export class AmqpSocket
  ...
  constructor(conn: Deno.Conn) {
    this.#conn = conn;
    this.#reader = createFrameReader(conn);
  }
  ...
  async write(frame: OutgoingFrame): Promise<void> {
    ...
    await this.#conn.write(write(frame));
  }
